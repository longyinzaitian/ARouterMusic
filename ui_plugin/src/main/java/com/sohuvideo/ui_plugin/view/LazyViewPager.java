...
public class LazyViewPager extends ViewGroup {
    ...
    public void setOffscreenPageLimit(int limit) {
        if (limit < DEFAULT_OFFSCREEN_PAGES) {
            LogManager.w(TAG, "Requested offscreen page limit " + limit
    ...
    void populate() {
        ...
        if (mPopulatePending) {
            if (DEBUG) {
                LogManager.i(TAG, "populate is pending, skipping for now...");
            }
            return;
        }
        ...
        if (DEBUG) {
            LogManager.v(TAG, "populating: startPos=" + startPos + " endPos=" + endPos);
        }
        ...
        for (int i = 0; i < mItems.size(); i++) {
            ...
            if ((ii.position < startPos || ii.position > endPos)
                    && !ii.scrolling) {
                if (DEBUG) {
                    LogManager.i(TAG, "removing: " + ii.position + " @ " + i);
                }
                mItems.remove(i);
                i--;
                mAdapter.destroyItem(this, ii.position, ii.object);
                ...
                while (lastPos <= endPos && lastPos < ii.position) {
                    if (DEBUG) {
                        LogManager.i(TAG, "inserting: " + lastPos + " @ " + i);
                    }
        ...
        if (lastPos < endPos) {
            ...
            while (lastPos <= endPos) {
                if (DEBUG) {
                    LogManager.i(TAG, "appending: " + lastPos);
                }
        ...
        if (DEBUG) {
            LogManager.i(TAG, "Current page list:");
            for (int i = 0; i < mItems.size(); i++) {
                LogManager.i(TAG, "#" + i + ": page " + mItems.get(i).position);
            }
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < size; ++i) {
            ...
            if (child.getVisibility() != GONE) {
                if (DEBUG) {
                    LogManager.v(TAG, "Measuring #" + i + " " + child + ": "
    ...
    public void computeScroll() {
        if (DEBUG) {
            LogManager.i(TAG, "computeScroll: finished=" + mScroller.isFinished());
        }
        if (!mScroller.isFinished()) {
            if (mScroller.computeScrollOffset()) {
                if (DEBUG) {
                    LogManager.i(TAG, "computeScroll: still scrolling");
                }
                int oldX = getScrollX();
                int oldY = getScrollY();
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (action == MotionEvent.ACTION_CANCEL
            ...
            // Release the drag.
            if (DEBUG) {
                LogManager.v(TAG, "Intercept done!");
            }
        ...
        if (action != MotionEvent.ACTION_DOWN) {
            if (mIsBeingDragged) {
                if (DEBUG) {
                    LogManager.v(TAG, "Intercept returning true!");
                }
                return true;
            }
            if (mIsUnableToDrag) {
                if (DEBUG) {
                    LogManager.v(TAG, "Intercept returning false!");
                }
                return false;
            }
        }
        ...
        switch (action) {
			 ...
			 */
                ...
                if (DEBUG) {
                    LogManager.v(TAG, "Moved x to " + x + "," + y + " diff=" + xDiff + ","
                            + yDiff);
                }
                ...
                if (xDiff > mTouchSlop && xDiff > yDiff) {
                    if (DEBUG) {
                        LogManager.v(TAG, "Starting drag!");
                    }
                    ...
                    if (yDiff > mTouchSlop) {
                        ...
                        if (DEBUG) {
                            LogManager.v(TAG, "Starting unable to drag!");
			 ...
			 */
                ...
                if (DEBUG) {
                    LogManager.v(TAG, "Down at " + mLastMotionX + "," + mLastMotionY
